...
1. A new column called isWeekend is created. This is set to TRUE if the weekday is Saturday or Sunday and FALSE to every othe weeekday.

```{r, echo=TRUE}
for (i in seq_along(data_filled_missing_values)) {
    if (weekdays(as.Date(data_filled_missing_values[i,2])) == "Saturday") {
        data_filled_missing_values$isWeekend <- TRUE
    }
    else if (weekdays(as.Date(data_filled_missing_values[i,2])) == "Sunday") {
        data_filled_missing_values$isWeekend <- TRUE
    }
   else {
       data_filled_missing_values$isWeekend <- FALSE
   }
}
